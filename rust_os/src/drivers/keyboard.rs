...
use crate::arch::x86::port::inb;
use crate::printk;
// Remove VGA buffer dependencies
// use crate::vga_buffer::vga_buffer::{WRITER, BUFFER_HEIGHT, BUFFER_WIDTH};
// use crate::vga_buffer::{WRITER, BUFFER_HEIGHT, BUFFER_WIDTH};

// Replace with screen manager dependencies
...
pub fn handle_backspace() {
    ...
    if let Some(active_screen) = &mut manager.screens[active_screen_id] {
        ...
        if col > 0 {
            active_screen.set_column_position(col - 1);
            active_screen.write_byte(b' '); // Write a space to clear the character
            active_screen.set_column_position(col - 1); // Move cursor back to the position
        }
    }
    manager.update_cursor();
}
...
//     let active_screen_id = manager.active_screen_id;
//     if let Some(active_screen) = &mut manager.screens[active_screen_id] {
//         let current_col = active_screen.column_position;
//         let current_row = active_screen.row_position;
...
//             // This is a basic implementation - you might want to enhance it
//             // to shift characters left for a more sophisticated text editor
//             active_screen.write_byte(b' '); // Clear current character
//             active_screen.set_column_position(current_col); // Reset cursor to original position
//         }
//     }
//     manager.update_cursor();
// }
